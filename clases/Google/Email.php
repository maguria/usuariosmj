...
class Email {
    ...
    function __construct($destino=null,  $asunto=null, $mensaje=null, $origen=null, $alias=null) {
        ...
        $this->cliente->setAccessToken(file_get_contents($_SERVER['DOCUMENT_ROOT'].'oauth/token.conf'));
        $this->origen= $origen;
    ...
    function enviar(){
        if ($this->cliente->getAccessToken()) {
            ...
            try {
                $mail = new PHPMailer();
                $mail->CharSet = "UTF-8";
                $mail->From = Constant::FROM;
                $mail->FromName = Constant::ALIAS;
                $mail->AddAddress($this->destino);
                $mail->AddReplyTo(Constant::FROM, Constant::ALIAS);
